...
function(a) {
	...
	f.addInterceptor(function(a, b, c) {
        ...
        var getqocde = ''
        if(a.properties.src == 'syspic/pic/6/201603/56e6755bb46b7.jpeg') {
            getqocde = ' getqocde'
        }
		return b.id = a.id, b.setAttribute("ctype", a.type), b.setAttribute("class", "element comp_image editable-image" + getqocde), /^http.*/.test(a.properties.src) ? b.src = a.properties.src : b.src = PREFIX_FILE_HOST + a.properties.src, b
		...
		if (a = $("button", a)[0], "view" == f.mode) {
			var c = function(b, c) {
					...
					$("textarea", e).each(function() {
						...
						cache: !0,
						type: "POST",
						url: PREFIX_URL + "index.php?c=scenedata&a=add&id=" + c+"&pageid="+myobj.pageId,
						data: $.param(f),
						async: !1,
						error: function(a) {
							alert("Connection error")
...
function(a, b) {
	...
	function d(a) {
		...
		var c = PREFIX_HOST + "index.php?c=statics&a=getExposeTypes";
		b.ajax({
			type: "GET",
			url: c,
			xhrFields: {
		...
		m = 2, b.ajax({
			...
			success: function(e) {
				...
				for (elements.length; h < elements.length; h++) {
					...
					if (i.sceneId = a.obj.id, i.pageId = pages[pages.length - 1].id, a.obj.property.bottomLabel.name && a.obj.property.bottomLabel.url && "http://" != a.obj.property.bottomLabel.url) 2 == i.type && i.content.indexOf("WESAMBO科技公司") > 0 && (i.content = i.content.replace(/WESAMBO科技公司/, '<a href="' + PREFIX_S2_URL + "?c=scene&a=link&id=" + a.obj.id + "&amp;url=" + encodeURIComponent(a.obj.property.bottomLabel.url) + '" target=_blank>' + a.obj.property.bottomLabel.name + "</a>"));
					else if (a.obj.property.bottomLabel.name) 2 == i.type && i.content.indexOf("WESAMBO科技公司") > 0 && (i.content = i.content.replace(/WESAMBO科技公司/, a.obj.property.bottomLabel.name));
					else if (/一秀技术支持/.test(i.content)) {
			...
			addElmentsForPc = function(a) {
				...
				c.href = CLIENT_CDN + "pcviewer.css", c.rel = "stylesheet", b.appendChild(c), window != window.top ? $("body").css("background-image", "none") : (getBg(), $.getScript(CLIENT_CDN + "qrcode.js", function() {
					$.getScript(CLIENT_CDN + "jquery.qrcode.js", function() {
						$("#con").before('<div id="code"><div>扫一扫，分享给朋友！</div><div style="text-align: center;background:#fff;padding: 10px;" id="codeImg"/><div id="view_share" class="bdsharebuttonbox"><a href="#" class="bds_more" data-cmd="more"></a><a href="#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a><a href="#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a><a href="#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a><a href="#" class="bds_sqq" data-cmd="sqq" title="分享到QQ好友"></a><a href="#" class="bds_douban" data-cmd="douban" title="分享到豆瓣网"></a></div><div id="view_reg">这么漂亮的场景&nbsp;→<span><a target="_blank" href="'+web_site+'">我也来制作</a></span></div><div id="support">技术支持 BY <a target="_blank" href="'+web_site+'"><img id="logoSmall" src="'+web_logo+'"/></a></div></div>');
						var b;
