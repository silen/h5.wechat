...
class Collect_70cController extends BaseController {
	public function index(){
 		$this->display('Scene/collect_70c' );
    }
	...
	public function collect()
    {
        ...
        $eqxcj = 'http://www.70c.com/As/getAs?sceneid='.$getcs.'&objID=&p=0';
        ...
        if (empty($cj_int) and $cj_name[obj][name] !== '该场景已关闭') {
            ...
            $preg_replace = preg_replace('/cloudapppic(\\/\\w+)*\\//', $cj_syspic, $eqxcjt);
            $json_decode = json_decode($preg_replace, true);
            foreach ($preg_pic[0] as $zym_50 => $zym_45) {
                $zym_44[$zym_50] = pathinfo($zym_49[0][$zym_50]);
                $this->save_pic('http://linkappstatic.wydapp.com/' . $zym_45, $scenepic);
            }
			//$where['sceneid_bigint'] = $cj_name['obj']['id'];
            $where['scenename_varchar'] = $cj_name['obj']['name'];
            $where['scenecode_varchar'] =  'C'.(date('y',time())-9).date('m',time()).randorderno(6,-1);
			$tmp = $where['scenecode_varchar'];
			...
			if (!empty($cj_name['obj']['image']['bgAudio']['url'])) {
                if (preg_match('|^http://|', $cj_name['obj']['image']['bgAudio']['url'])) {
                    ...
                    $zym_41 = 'http://linkappstatic.wydapp.com/' . $cj_name['obj']['image']['bgAudio']['url'];
                }
                ...
                if (preg_match('|http://|', $cj_name['obj']['bgAudio'])) {				
					...
					$arr = explode("\"",$cj_name['obj']['bgAudio']);
                    $zym_41 = 'http://linkappstatic.wydapp.com/' . $arr[3];
                }
                $where['musicurl_varchar'] = 'syspic/mp3/' . $this->save_pic($zym_41, $eqxmp3);
            }*/
            ...
            $zym_42 = 'http://linkappstatic.wydapp.com/' . $cj_name['obj']['image']['imgSrc'];
            ...
            $where['musictype_int'] = empty($cj_name['obj']['image']['bgAudio']['type']) ? 'null' : $where['musictype_int'];
			 ...
			 //echo json_encode(array('msg' => $json_decode['list']));
			//exit();
